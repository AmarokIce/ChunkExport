...
public class ChunkProcessor extends Thread {
    private final Map<Block, Map<Integer, Integer>> blockOnLevelMap = new HashMap<>();
    private final Map<Block, BlockInfo> blockInfoMap = new HashMap<>();
    private final Map<Block, String> blockNameMap = new HashMap<>();

    private List<ChunkAccess> list;
    ...
    public void run() {
        ...
        for (ChunkAccess chunk : this.list) {
            player.sendMessage(new TranslatableComponent("Remaining chunk : " + --chunkCount), player.getUUID());
            for (int y = 1; y < 256; y++)
        ...
        player.sendMessage(new TranslatableComponent("chunk_export.success.message"), player.getUUID());
    }
    ...
    private void JsonHelper() {
        player.sendMessage(new TranslatableComponent("chunk_export.operation.message"), player.getUUID());
        ...
        player.sendMessage(new TranslatableComponent("chunk_export.output.message"), player.getUUID());
        new JsonHandler(this.level.dimension().location().getPath().toString());
    }
